        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="myModalLabel">Editar Datos del nro de Cuenta</h4>
                </div>
    ...
    <div class="modal fade" id="deleteReq" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title" id="myModalLabel">Esta Seguro de Eliminar este Nro de Cuenta</h4>
                </div>
                <div class="modal-body">
                    <input type="hidden" name="_token" value="{{ csrf_token() }}" id="token"/>
                    <input type="hidden" id="id">
        ...
        <script>
            ...
            function load(){
                var data=$('#datos');
                var route="http://localhost:8000/cuentaList";
                data.empty();
                $.get(route, function(res){
                    $(res).each(function(key, value){
                        data.append("<tr><td>"+value.entidad_bancaria+"</td><td>"+value.nro+"</td><td><button class='btn btn-info' value="+value.id+" onclick='mostrar(this)' data-toggle='modal'  data-target='#editReq'><i class='fa fa-pencil fa-lg'></i> Editar</button><button class='btn btn-danger' value="+value.id+" onclick='beforeDelete(this)' data-toggle='modal'  data-target='#deleteReq'><i class='fa fa-trash-o fa-lg'></i> Eliminar</button></td></tr>");
                    });
                });
            };
            var idd=0;
            ...
            function eliminar(value){
                var route="http://localhost:8000/cuenta/"+value+"";
                var token=$('#token').val();
                $.ajax({
                    url:route,
            ...
            function mostrar(btn){
                ...
                var route="http://localhost:8000/cuenta/"+btn.value+"/edit";
                $.get(route, function(res){
                    $('#name').val(res.entidad_bancaria);
                    $('#description').val(res.nro);
                    $('#id').val(res.id);
                });
            };
            $('#update').click(function(){
                ...
                var route="http://localhost:8000/cuenta/"+id+"";
                var token=$('#token').val();
                $.ajax({
                    url:route,
                    headers:{'X-CSRF-TOKEN' : token},
                    type:'PUT',
                    dataType:'json',
                    data:{entidad_bancaria:name,nro:desc},
                    success: function(){
                        load();
